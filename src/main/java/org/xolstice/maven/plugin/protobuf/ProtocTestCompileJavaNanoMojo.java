...
/**
 * This mojo executes the {@code protoc} compiler for generating test JavaNano sources from protocol
 * buffer definitions. It also searches dependency artifacts in the test scope for {@code .proto}
 * files and includes them in the {@code proto_path} so that they can be referenced. Finally, it
 * adds the {@code .proto} files to the project as test resources so that they can be included in
 * the test-jar artifact.
 *
 * @since 0.4.3
 */
@Mojo(name = "test-compile-javanano", defaultPhase = LifecyclePhase.GENERATE_TEST_SOURCES,
...
public final class ProtocTestCompileJavaNanoMojo extends AbstractProtocTestCompileMojo {
   ...
   */
  @Parameter(required = true, defaultValue = "${project.build.directory}/generated-test-sources/protobuf/javanano")
  private File outputDirectory;

  /**
   * Additional comma-separated options to be passed to the JavaNano generator. <b>Cannot</b>
   * contain colon (<tt>:</tt>) symbols.
   */
  @Parameter(required = false, property = "javaNanoOptions")
