 ...
 *
 * <p>
 * This class is extended by {@link ProtocCompileMojo} and {@link ProtocTestCompileMojo} in order to
 * override the specific configuration for compiling the main or test classes respectively.
 * </p>
 */
abstract class AbstractProtocMojo extends AbstractMojo {
  ...
  protected enum MojoType {
    DEFAULT, CPP, CUSTOM, J2OBJC, JAVA_NANO, PYTHON;
  };
  ...
  /**
   * This is the path to the {@code protoc} executable. When this parameter is not set, the plugin
   * attempts to load a {@code protobuf} toolchain and use it locate {@code protoc} executable. If
   * no {@code protobuf} toolchain is defined in the project, the {@code protoc} executable in the
   * {@code PATH} is used.
   */
  ...
  /**
   * Protobuf compiler artifact specification, in
   * {@code groupId:artifactId:version[:type[:classifier]]} format. When this parameter is set, the
   * plugin attempts to resolve the specified artifact as {@code protoc} executable.
   *
   * @since 0.4.1
   */
  ...
  /**
   * Since {@code protoc} cannot access jars, proto files in dependencies are extracted to this
   * location and deleted on exit. This directory is always cleaned during execution.
   */
   ...
   * <p/>
   * This plugin expands jars on the classpath looking for embedded {@code .proto} files. Normally
   * these paths are hashed (MD5) to avoid issues with long file names on windows. However if this
   * property is set to {@code false} longer paths will be used.
   */
  @Parameter(required = true, defaultValue = "true")
  private boolean hashDependentPaths;

  /**
   * A list of &lt;include&gt; elements specifying the protobuf definition files (by pattern) that
   * should be included in compilation. When not specified, the default includes will be:
   * <code><br/>
   * &lt;includes&gt;<br/>
  ...
  /**
   * A list of &lt;exclude&gt; elements specifying the protobuf definition files (by pattern) that
   * should be excluded from compilation. When not specified, the default excludes will be empty:
   * <code><br/>
   * &lt;excludes&gt;<br/>
   * &lt;/excludes&gt;<br/>
   * </code>
  ...
  /**
   * If set to {@code true}, then the specified protobuf source files from this project will be
   * attached as resources to the build, for subsequent inclusion into the final artifact. This is
   * the default behaviour, as it allows downstream projects to import protobuf definitions from the
   * upstream projects, and those imports are automatically resolved at build time.
   *
   * <p>
   * If distribution of {@code .proto} source files is undesirable for security reasons or because
   * of other considerations, then this parameter should be set to {@code false}.
   * </p>
   *
   * @since 0.4.1
   */
  ...
  /**
   * If {@code true} and {@code writeDescriptorSet} has been set, the compiler will include all
   * dependencies in the descriptor set making it "self-contained".
   *
   * @since 0.3.0
   */
  ...
  /**
   * If {@code true} and {@code writeDescriptorSet} has been set, do not strip SourceCodeInfo from
   * the FileDescriptorProto. This results in vastly larger descriptors that include information
   ...
   * An executable plugin will be created at execution time. On UNIX the executable is a shell
   * script and on Windows it is a WinRun4J .exe and .ini.
   */
  @Parameter(required = false)
  private List<ProtocPlugin> protocPlugins;

  /**
   * Sets the granularity in milliseconds of the last modification date for testing whether source
   * protobuf definitions need recompilation.
   *
   * <p>
  ...
  /**
   * Normally {@code protoc} is invoked on every execution of the plugin. Setting this parameter to
   * {@code true} will enable checking timestamps of source protobuf definitions vs. generated
   * sources.
   *
   * @see #staleMillis
   */
  ...
  /**
   * Usually most of protobuf mojos will not get executed on parent poms (i.e. projects with
   * packaging type 'pom'). Setting this parameter to {@code true} will force the execution of this
   * mojo, even if it would usually get skipped in this case.
   *
   * @since 0.2.0
   */
  ...
  /**
   * When {@code true}, the output directory will be cleared out prior to code generation. With the
   * latest versions of protoc (2.5.0 or later) this is generally not required, although some
   * earlier versions reportedly had issues with running two code generations in a row without
   * clearing out the output directory in between.
   *
   * @since 0.4.0
   */
      ...
      try {
        ...
        if (protoFiles.isEmpty()) {
          ...
          if (tc != null) {
            ...
            // when the executable to use is explicitly set by user in mojo's parameter, ignore
            // toolchains.
          ...
          final Protoc.Builder protocBuilder =
              new Protoc.Builder(protocExecutable).addProtoPathElement(protoSourceRoot).addProtoPathElements(derivedProtoPathElements)
                  .addProtoPathElements(asList(additionalProtoPathElements)).addProtoFiles(protoFiles);
          addProtocBuilderParameters(protocBuilder);
          protocBuilder.setMojoType(getMojoType());
  ...
  /**
   * Generates native launchers for java protoc plugins. These launchers will later be added as
   * parameters for protoc compiler.
   *
  ...
  protected void createProtocPlugins() throws MojoExecutionException {
    ...
    for (final ProtocPlugin plugin : protocPlugins) {
      ...
      final ProtocPluginAssembler assembler =
          new ProtocPluginAssembler(plugin, session, project.getArtifact(), artifactFactory, repositorySystem, resolutionErrorHandler,
              localRepository, remoteRepositories, protocPluginDirectory, getLog());
      assembler.execute();
    }
  }

  /**
   * Attempts to detect java home directory, using {@code jdk} toolchain if available, with a
   * fallback to {@code java.home} system property.
   *
  ...
  protected void addProtocBuilderParameters(final Protoc.Builder protocBuilder) throws MojoExecutionException {
    ...
    if (writeDescriptorSet) {
      ...
      protocBuilder.withDescriptorSetFile(descriptorSetFile, includeDependenciesInDescriptorSet, includeSourceInfoInDescriptorSet);
    }
  }
   ...
   * @return {@code true}, if source files are not later than the target files; {@code false},
   *         otherwise.
   */
   ...
   * @return {@code true}, if at least one file has changes; {@code false}, if no files have
   *         changes.
   *
   * @since 0.3.0
   */
  ...
  /**
   * 
   * @return the mojo type that was selected
   */
  protected abstract MojoType getMojoType();
  ...
  /**
   * Returns the output directory for generated sources. Depends on build phase so must be defined
  ...
  /**
   * Returns output directory for descriptor set file. Depends on build phase so must be defined in
  ...
  /**
   * Unpacks proto descriptors that are bundled inside dependent artifacts into a temporary
   * directory. This is needed because protobuf compiler cannot handle imported descriptors that are
   * packed inside jar files.
   *
   ...
   * @return a set of import roots for protobuf compiler (these will all be subdirectories of the
   *         temporary directory).
   */
  protected ImmutableSet<File> makeProtoPathFromJars(final File temporaryProtoFileDirectory, final Iterable<File> classpathElementFiles)
    ...
    for (final File classpathElementFile : classpathElementFiles) {
      ...
      // I am excluding .xml rather than including .jar as there may be other extensions in use
      // (sar, har, zip)
      if (classpathElementFile.isFile() && classpathElementFile.canRead() && !classpathElementFile.getName().endsWith(".xml")) {
        ...
        try {
          ...
          throw new IllegalArgumentException(format("%s was not a readable artifact", classpathElementFile), e);
        }
  ...
  protected ImmutableSet<File> findProtoFilesInDirectory(final File directory) throws IOException {
    ...
    final List<File> protoFilesInDirectory = getFiles(directory, joiner.join(getIncludes()), joiner.join(getExcludes()));
  ...
  protected File resolveBinaryArtifact(final Artifact artifact) throws MojoExecutionException {
    ...
    try {
      final ArtifactResolutionRequest request =
          new ArtifactResolutionRequest().setArtifact(project.getArtifact()).setResolveRoot(false).setResolveTransitively(false)
              .setArtifactDependencies(Collections.singleton(artifact)).setManagedVersionMap(Collections.emptyMap())
              .setLocalRepository(localRepository).setRemoteRepositories(remoteRepositories).setOffline(session.isOffline())
              .setForceUpdate(session.getRequest().isUpdateSnapshots()).setServers(session.getRequest().getServers())
  ...
  protected Artifact createDependencyArtifact(final String artifactSpec) throws MojoExecutionException {
    ...
    if (parts.length < 3 || parts.length > 5) {
      throw new MojoExecutionException("Invalid artifact specification format"
          + ", expected: groupId:artifactId:version[:type[:classifier]]" + ", actual: " + artifactSpec);
    }
  ...
  protected Artifact createDependencyArtifact(final String groupId, final String artifactId, final String version, final String type,
    ...
    return artifactFactory.createDependencyArtifact(groupId, artifactId, versionSpec, type, classifier, Artifact.SCOPE_RUNTIME);
  }
}
