...
/**
 * This mojo executes the {@code protoc} compiler with the specified plugin executable to generate
 * test sources from protocol buffer definitions. It also searches dependency artifacts for
 * {@code .proto} files and includes them in the {@code proto_path} so that they can be referenced.
 * Finally, it adds the {@code .proto} files to the project as resources so that they can be
 * included in the test-jar artifact.
 *
 * @since 0.4.1
 */
@Mojo(name = "test-compile-custom", defaultPhase = LifecyclePhase.GENERATE_TEST_SOURCES,
    requiresDependencyResolution = ResolutionScope.TEST, threadSafe = true)
public final class ProtocTestCompileCustomMojo extends AbstractProtocTestCompileMojo {

  /**
   * A unique id that identifies the plugin to protoc. <strong>Cannot</strong> be one of the
   * built-in protoc plugins:
   * <ul>
   * <li>java</li>
   * <li>javanano</li>
   * <li>cpp</li>
  ...
  /**
   * This is the base directory for the generated code. If an explicit {@link #outputDirectory}
   * parameter is not specified, an output directory named after {@link #pluginId} will be created
   * inside this base directory.
   */
  @Parameter(required = true, readonly = true, defaultValue = "${project.build.directory}/generated-test-sources/protobuf")
  private File outputBaseDirectory;

  /**
   * This is the directory where the generated code will be placed. If this parameter is
   * unspecified, then the default location is constructed as follows:<br>
  ...
  /**
   * An optional path to plugin executable. If unspecified, alternative options must be used (e.g.
   * toolchains).
   */
  ...
  /**
   * An optional parameter to be passed to the plugin. <b>Cannot</b> contain colon (<tt>:</tt>)
   * symbols.
   */
  ...
  /**
   * A name of an optional custom toolchain that can be used to locate the plugin executable. The
   * toolchain must be registered as a build extension and initialised properly.
   */
  ...
  /**
   * Plugin artifact specification, in {@code groupId:artifactId:version[:type[:classifier]]}
   * format. When this parameter is set, the specified artifact will be resolved as a plugin
   * executable.
   *
   * @since 0.4.1
   */
